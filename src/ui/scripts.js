...
// FormHub Worker Frontend SPA Application
let state = {
...
function updateAuthUI() {
  ...
  if (state.user && state.token) {
    ...
    navAuth.innerHTML = \`
      ...
      <button class="btn btn-secondary btn-sm" onclick="logout(true)">Log Out</button>
      <button class="btn btn-icon btn-sm" id="theme-toggle-btn" onclick="toggleTheme()" title="Toggle Theme">🌙 Dark</button>
    \`;
  } else {
    navLinks.innerHTML = \`
      <a class="nav-link" href="#home">Home</a>
...
function renderHomeView(container) {
  container.innerHTML = \`
    <section class="hero-section">
      <div class="hero-badge">⚡ Cloudflare Worker Edge + MongoDB Atlas Source of Truth</div>
      <h1 class="hero-title">Multi-Tenant Form Intake on <span class="gradient-text">Cloudflare Edge</span></h1>
      <p class="hero-subtitle">
        Create custom form schemas, collect secure multi-part submissions, and sync data instantly as Shopify Metaobjects—all powered by a single serverless worker.
      </p>
      ...
      <div class="features-grid">
        <div class="feature-card">
          ...
          <p class="feature-description">
            Your form endpoints live at <code style="color:#818cf8">/:username/:appname/</code> with built-in CORS, IP rate limiting, Turnstile verification, and honeypot protection.
          </p>
        </div>
        <div class="feature-card">
    ...
    <div style="max-width: 900px; margin: 0 auto; padding: 1rem 0;">
      <div class="guide-banner">
        <div class="guide-banner-text">
          <h4>🛍️ Shopify Metaobject Integration Guide</h4>
      ...
      <div class="app-card">
        <div>
          ...
          <div class="app-card-meta" style="margin-top: 1rem;">
            ...
            \${shopifyConfigured ? \`
              <span class="badge badge-shopify" title="Metaobject Type: \${escapeHtml(app.settings.shopify.metaobjectType)}">🛍️ Shopify Sync ON</span>
            \` : \`
              <span class="badge badge-disabled">🛍️ Shopify OFF</span>
            \`}
            \${turnstileConfigured ? \`
              <span class="badge badge-turnstile">🛡️ Turnstile ON</span>
            \` : ''}
        ...
        <div class="app-card-footer">
          <div class="app-card-actions">
            <button class="btn btn-secondary btn-sm" onclick="openEditAppModal('\${escapeHtml(app.appName)}')">⚙️ Manage</button>
          ...
          <button class="btn btn-danger btn-sm" onclick="deleteApp('\${escapeHtml(app.appName)}')" title="Delete App">🗑️</button>
        </div>
      </div>
    \`;
  }).join('');
...
function applyTemplatePreset(preset) {
  ...
  showToast('⚡ Applied preset: ' + (title || 'Blank Form'), 'info');
}
    ...
    <div class="field-card-item" data-idx="\${idx}">
      ...
      <div class="field-card-header">
        <div style="display:flex; align-items:center; gap:0.75rem;">
          <span class="field-type-pill">\${escapeHtml(typeBadge)}</span>
      ...
      <div class="field-card-grid-top">
        ...
        <div>
          ...
          <select class="form-select field-type" onchange="handleFieldTypeChange(this, \${idx})">
            ...
            <option value="email" \${f.type === 'email' ? 'selected' : ''}>✉️ Email Address</option>
      ...
      <div class="field-card-grid-advanced">
        <div>
          <label class="form-label" style="font-size:0.75rem; color:#93c5fd; margin-bottom:0.2rem;">⚙️ Regex Validation Pattern</label>
...
function handleFieldTypeChange(selectEl, idx) {
  ...
  if (pill) {
    ...
    pill.innerText = typeLabelMap[val] || val.toUpperCase();
  }
  updateImageFieldDropdown();
}
...
function openApiHubModal(appName) {
  ...
  const submitUrl = origin + '/' + encodeURIComponent(state.user.username) + '/' + encodeURIComponent(app.appName) + '/';
...
function buildCurlSnippet(app, submitUrl) {
  ...
  fields.forEach((f, i) => {
    const flag = f.type === 'file' ? '-F "' + f.key + '=@/path/to/photo.jpg"' : '-F "' + f.key + '=Sample ' + escapeHtml(f.label) + '"';
  ...
  container.innerHTML = list.map(s => {
    ...
    if (s.files && Object.keys(s.files).length > 0) {
      filesHtml = Object.entries(s.files).map(([k, fileObj]) => {
        if (!fileObj) return '';
        if (fileObj.storage === 'r2' && fileObj.url) {
          return '<a href="' + fileObj.url + '" target="_blank" style="color:#818cf8; text-decoration:underline;">🖼️ ' + escapeHtml(k) + '</a>';
        } else if (fileObj.storage === 'shopify') {
          return '<span style="color:#34d399;">🛍️ Shopify ID: ' + escapeHtml(fileObj.shopifyFileId) + '</span>';
        }
        return '';
      }).join('<br/>');
    }
...
async function inspectSubmissionStatus(submissionId) {
  ...
  try {
    ...
    document.getElementById('inspect-modal-title').innerText = 'Submission Details: ' + s.id;
    
    let detailsHtml = \`
      <div style="margin-bottom: 1.25rem;">
        <h4 style="margin-bottom:0.5rem; color:var(--text-secondary);">Sync Status</h4>
        <span class="status-badge status-\${s.shopifyStatus || 'skipped'}">\${(s.shopifyStatus || 'SKIPPED').toUpperCase()}</span>
        \${s.shopifyHandle ? '<p style="margin-top:0.5rem;">Shopify Handle: <code style="color:#34d399;">' + escapeHtml(s.shopifyHandle) + '</code></p>' : ''}
      </div>
    \`;

    if (s.shopifyErrors && s.shopifyErrors.length > 0) {
      detailsHtml += \`
        <div style="margin-bottom: 1.25rem; padding: 1rem; background: var(--danger-bg); border-radius: var(--radius-md);">
          <h4 style="color:var(--danger-color); margin-bottom:0.5rem;">⚠️ Shopify Sync Errors</h4>
          <ul style="margin-left: 1.5rem; color:#f8fafc; font-family: var(--font-mono); font-size:0.85rem;">
            \${s.shopifyErrors.map(err => '<li>' + escapeHtml(err.message || JSON.stringify(err)) + '</li>').join('')}
          </ul>
        </div>
      \`;
    }
  ...
  html += fields.map(f => {
    ...
    if (f.type === 'select') {
      ...
      return \`
        <div class="form-group\${customClassStr}" style="\${groupStyle}">
          <label class="form-label">\${escapeHtml(f.label)}\${reqStar}</label>
          <input type="file" name="\${f.key}" class="form-input" accept="image/*" \${reqStr} />
          <span class="form-hint" style="display:block; margin-top:0.25rem;">Max file size: \${Math.round((app.settings?.maxFileBytes || 10485760)/1048576)} MB. \${escapeHtml(f.helpText || '')}</span>
        </div>
      \`;
    } else {
      let typeStr = 'text';
...
async function submitLiveTestForm(e) {
  ...
  const submitUrl = '/' + encodeURIComponent(state.user.username) + '/' + encodeURIComponent(app.appName) + '/';
  ...
  try {
    ...
    if (response.ok && result.ok) {
      ...
      if (result.redirectUrl) {
        msgHtml += '<div style="margin-top:0.4rem; font-size:0.8rem; color:var(--text-secondary);">Redirect configured to: <a href="' + escapeHtml(result.redirectUrl) + '" target="_blank" style="color:#818cf8; text-decoration:underline;">' + escapeHtml(result.redirectUrl) + '</a></div>';
      }
      resBox.innerHTML = msgHtml;
      form.reset();
    } else {
      resBox.innerHTML = '<span style="color:var(--danger-color);">⚠️ Submission Error:</span> <code>' + escapeHtml(result.error || 'Unknown error') + '</code>';
    }
  } catch (err) {
  ...
  try {
    ...
    if (data.ok && data.token) {
      ...
      if (data.notRegistered || (data.error && data.error.toLowerCase().includes('no account'))) {
        ...
        errorBox.innerHTML = '⚠️ <strong>Database Error:</strong> ' + data.error + '<br><small style="color:var(--text-secondary); margin-top:0.3rem; display:block;">Tip: Verify IP (0.0.0.0/0) is whitelisted in Atlas Network Access and MONGODB_URI in .dev.vars is valid.</small>';
      } else {
