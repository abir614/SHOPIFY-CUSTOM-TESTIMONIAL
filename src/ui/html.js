...
<head>
  ...
  <meta name="description" content="Serverless multi-tenant form intake platform on Cloudflare Workers and MongoDB with optional Shopify Admin API metaobject sync." />
...
<body>
  ...
  <header class="app-header">
    <a class="logo-group" href="#home">
      ...
      <span class="logo-badge">Edge Worker</span>
    </a>
  ...
  <div id="app-editor-modal" class="modal-overlay">
    <div class="modal-container" style="max-width: 820px;">
      <div class="modal-header">
        ...
        <button class="modal-close" onclick="closeModal('app-editor-modal')">✕</button>
      </div>
      <div class="modal-body">
        ...
        <div style="background:rgba(129,140,248,0.08); border:1px solid rgba(129,140,248,0.25); border-radius:var(--radius-md); padding:0.75rem 1rem; margin-bottom:1.2rem; display:flex; flex-wrap:wrap; align-items:center; gap:0.5rem;">
          <span style="font-size:0.8rem; font-weight:700; color:var(--accent-color); text-transform:uppercase; letter-spacing:0.05em; margin-right:0.3rem;">⚡ Template Presets:</span>
          <button type="button" class="btn btn-secondary btn-sm" onclick="applyTemplatePreset('contact')">📬 Contact Form</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="applyTemplatePreset('shopify-story')">🛍️ Shopify Product Story</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="applyTemplatePreset('survey')">📊 Customer Survey</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="applyTemplatePreset('event')">🎟️ Event Registration</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="applyTemplatePreset('blank')" style="border-color:var(--border-color); color:var(--text-secondary);">✨ Scratch / Blank</button>
        </div>

        <!-- Tabs Header -->
        <div class="tabs-header">
          <button type="button" class="tab-btn app-tab-btn active" data-tab="general" onclick="switchAppModalTab('general')">🎨 1. General & UI Theme</button>
          <button type="button" class="tab-btn app-tab-btn" data-tab="fields" onclick="switchAppModalTab('fields')">📋 2. Fields Schema</button>
          <button type="button" class="tab-btn app-tab-btn" data-tab="shopify" onclick="switchAppModalTab('shopify')">🛍️ 3. Shopify Metaobjects</button>
          <button type="button" class="tab-btn app-tab-btn" data-tab="security" onclick="switchAppModalTab('security')">🛡️ 4. Security & Limits</button>
        </div>
        ...
        <div id="tab-pane-general" class="app-tab-pane">
          <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
            <div class="form-group">
              ...
              <span class="form-hint">API endpoint: <code style="color:#818cf8">/:username/:app-name/</code></span>
            </div>
            <div class="form-group">
        ...
        <div id="tab-pane-shopify" class="app-tab-pane" style="display:none;">
          <div class="guide-banner" style="margin-bottom:1.5rem;">
            <div class="guide-banner-text">
              <h4>🛍️ Shopify Metaobject Auto-Sync</h4>
  ...
  <div id="api-hub-modal" class="modal-overlay">
    <div class="modal-container" style="max-width: 800px;">
      <div class="modal-header">
        ...
        <button class="modal-close" onclick="closeModal('api-hub-modal')">✕</button>
      </div>
      <div class="modal-body">
        <div class="form-group">
    ...
    <div class="modal-container" style="max-width: 1000px;">
      <div class="modal-header">
        ...
        <div style="display:flex; gap:0.5rem; align-items:center;">
          ...
          <button class="modal-close" onclick="closeModal('submissions-modal')">✕</button>
        </div>
      </div>
  ...
  <div id="inspect-submission-modal" class="modal-overlay">
    <div class="modal-container" style="max-width: 650px;">
      <div class="modal-header">
        ...
        <button class="modal-close" onclick="closeModal('inspect-submission-modal')">✕</button>
      </div>
  ...
  <div id="live-test-modal" class="modal-overlay">
    <div class="modal-container" style="max-width: 580px;">
      <div class="modal-header">
        ...
        <button class="modal-close" onclick="closeModal('live-test-modal')">✕</button>
      </div>
      <form id="test-form" onsubmit="submitLiveTestForm(event)">
        <div class="modal-body">
          <p class="form-hint" style="margin-top:0; margin-bottom:1.25rem;">
            This renders your exact form schema. Submitting will send a live POST request to your Edge Worker endpoint and save it in MongoDB (and Shopify if enabled).
          </p>
